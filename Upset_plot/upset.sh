#!/bin/bash
#To get list of all genes of Saccacomyces cerevisae from pillers.tab
cat Pillars.tab |awk '$12!="---"||$22!="---"{print$12}' >12
cat Pillars.tab |awk '$12!="---"||$22!="---"{print$22}' >22
cat 12 22|grep -vE 'Scer_|---'|grep -v "Scer_" > Yeast
...
rm -v *[1234567890]*.output.tsv "$g".tsv "$g"1.list
rm WGD.2.main.sh main.preWGD.sh Input.output.tsv "$g".table.tsv "$g".list 12 22  "$g".raw_table.output.tsv "$g".matrix.tsv
